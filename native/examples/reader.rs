...
pub fn read_tickers(path: &str) -> Tickers {
    let tickers = File::open(&Path::new(path)).unwrap();
...
pub fn read_signals(path: &str) -> Signals {
    let signals = File::open(&Path::new(path)).unwrap();
